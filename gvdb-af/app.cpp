...
App sample_obj; // create an App object
...
void App::start_guis(int w, int h) {
	clearGuis();
	setview2D(w, h);
	guiSetCallback(handle_gui);
	addGui(20, h - 30, 130, 20, "Topology", GUI_CHECK, GUI_BOOL, &m_show_topo,
			0, 1);
	addGui(160, h - 30, 130, 20, "Halo", GUI_CHECK, GUI_BOOL, &m_halo, 0, 1);
	addGui(300, h - 30, 130, 20, "Sparse", GUI_CHECK, GUI_BOOL, &m_sparse, 0,
			1);
}

void App::setVolumeParams() {
	...
	gvdb.getScene()->LinearTransferFunc(0.00f, 0.15f, Vector4DF(0, 0, 0, 0),
			Vector4DF(0, 0, 0, 0.0f));
	gvdb.getScene()->LinearTransferFunc(0.15f, 0.25f, Vector4DF(0, 0, 0, 0),
			Vector4DF(0, 0, 1, 0.01f));			// skin range, blue
	gvdb.getScene()->LinearTransferFunc(0.25f, 0.50f, Vector4DF(0, 0, 1, 0.01f),
			Vector4DF(1, 0, 0, 0.02f));		// bone range, red
	gvdb.getScene()->LinearTransferFunc(0.50f, 0.75f, Vector4DF(1, 0, 0, 0.02f),
			Vector4DF(.2f, .2f, 0.2f, 0.02f));
	gvdb.getScene()->LinearTransferFunc(0.75f, 1.00f,
			Vector4DF(.2f, .2f, 0.2f, 0.02f), Vector4DF(.1, .1, .1, 0.1));
	gvdb.CommitTransferFunc();
}

void App::setCameraParams() {
...
// ************** Load data *************************************************************
void App::LoadRAW(char* fname, Vector3DI res, int bpp) {
	char buf[1024];
...
void App::Rebuild(Vector3DF vmax, bool bSparse, bool bHalo) {
	...
	//
	xform.SRT(Vector3DF(1, 0, 0), Vector3DF(0, 0, 1), Vector3DF(0, 1, 0),
			Vector3DF(0, 0, 252), Vector3DF(.5, -1, 1));
	...
	if (!bSparse) {
		...
		gvdb.DownsampleCPU(xform, m_DataRes, AUX_DATA3D, e.ires, vmax,
		AUX_DOWNSAMPLED, Vector3DF(0, 1, 0), Vector3DF(0, 1, 0));
	...
	printf("Resample.\n");
	gvdb.Resample(0, xform, m_DataRes, AUX_DATA3D, Vector3DF(0, 1, 0),
			Vector3DF(0, 1, 0));
}
...
bool App::init() {
	...
	gvdb.PrepareAux( AUX_DATA3D, m_DataRes.x * m_DataRes.y * m_DataRes.z,
...
void App::draw_topology() {
	...
	for (int lev = 0; lev < 5; lev++) {				// draw all levels
		...
		for (int n = 0; n < node_cnt; n++) {	// draw all nodes at this level
			...
			drawBox3D(bmin.x, bmin.y, bmin.z, bmax.x, bmax.y, bmax.z,
					clrs[lev].x, clrs[lev].y, clrs[lev].z, 1);
		}
	}
...
void App::display() {
	...
	if (m_show_topo)
		draw_topology();			// Draw GVDB topology
...
void App::keyboardchar(unsigned char key, int mods, int x, int y) {
	switch (key) {
	case '1':
...
void App::motion(int x, int y, int dx, int dy) {
	...
	case NVPWindow::MOUSE_BUTTON_RIGHT: {
		...
		dist -= dy;
		if (shift)
			lgt->setOrbit(lgt->getAng(), lgt->getToPos(), dist,
					cam->getDolly());
		else
			cam->setOrbit(cam->getAng(), cam->getToPos(), dist,
					cam->getDolly());
		postRedisplay();
	}
		break;
...
void App::mouse(NVPWindow::MouseButton button, NVPWindow::ButtonAction state,
...
int sample_main(int argc, const char** argv) {
	return sample_obj.run("NVIDIA(R) GVDB Voxels - gResample", "resample", argc,
			argv, 1024, 768, 4, 5);
}
