...
void saveVTK(string fileName, af::array outField)
{
    ofstream outFile(fileName.c_str(), ios::out | ios::out);
    outField.as(f32);
    int x,y,z =0;
    x = outField.dims(0);
    y = outField.dims(1);
    ...
    int totalNumVoxels =  x *  y *  z;

    //The VTK version 2.0 is used here:
    ...
    outFile << "POINT_DATA " << totalNumVoxels << "\n";
    outFile << "SCALARS " << fileName << " double 1" << "\n";
    outFile << "LOOKUP_TABLE default" << "\n";

    //Begin writing the data into the ASCII file:

    //Copy the content of ArrayFire array to C++ vector:
    float* outFieldData = outField.host<float>();

    size_t index = 0;
    for(int k = 0; k <  z; k++)
    for(int j = 0; j <  y; j++)
    for(int i = 0; i <  x; i++)
        outFile << outFieldData[index++] << "\n";
